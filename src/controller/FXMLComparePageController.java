...
public class FXMLComparePageController implements Initializable {
  ...
  @FXML private Label setAverageAll, setAverageUser;
  @FXML private Text txtUserScores;
  @FXML private ListView lvUserScores;
  @FXML private ListView lvPastScores;
  ...
  /**
   * Gets the current user's past test scores and displays them, up to the last 50 tests
   *
  ...
  /**
   * Shows the average of the user's last tests, as well as the past tests of every user
   *
  ...
  public void populateAverage(String user) {
    ...
    } else scoreFile = new File("src/datafiles/" + user + ".txt");
    try {
    ...
    if (user.equals("all")) setAverageAll.setText(String.format("%.2f", average) + "%");
    else {
